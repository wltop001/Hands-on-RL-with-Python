...
class Trainer:
    ...
    def train_loop(self, env, agent, episode=200, initial_count=-1, render=False, observe_interval=0):
        """
        进行学习的循环，在指定的episode环境中进行游戏，并根据回合的开始、结束以及各个步骤等运行相应的方法
        :param env:
        :param agent:
        :param episode: 当前训练的回合
        :param initial_count:
        :param render:
        :param observe_interval: 智能体在环境中运行的频率
        :return:
        """
        ...
        for i in range(episode):
            ...
            while not done:
                ...
                a = agent.policy(s)
                # 2.执行动作
                n_state, reward, done, info = env.stop()
                # 3.将交互记录加入经验池
